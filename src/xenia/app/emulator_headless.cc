          ...
          if (thread->thread_id() == 6) {
            ...
            // Function probes: check if key present-pipeline functions are JIT-compiled
            {
              ...
              static const FnProbe probes[] = {
                // CRT entry pipeline (updated 2026-02-25 from default.map)
                {0x8311ac20, "mainCRTStartup"},
                {0x82463930, "main()"},
                {0x8300d2ec, "App::App()"},
                // Init functions called by App::App (in order)
                {0x833a3b94, "DxRnd::PreInit(HWND)"},
                {0x833a40d0, "DxRnd::PreInit(virt)"},
                {0x83184310, "Rnd::PreInit"},
                {0x82a4b350, "Movie::Init"},
                {0x83438c9c, "BinkMovieSys::Init"},
                {0x833a404c, "Rnd::SetClearColor"},
                {0x834e24dc, "Splash::Splash(ctor)"},
                {0x82b20dd0, "OptionBool"},
                {0x83409780, "ULSystemLocale"},
                {0x834e2050, "Splash::PrepareNext"},
                {0x834e2dec, "Splash::BeginSplasher"},
                {0x834e21b8, "Splash::PrepareRemaining"},
                {0x832aaafc, "LiveCameraInput::PreInit"},
                {0x832aabe4, "LiveCameraInput::Init"},
                {0x832f69bc, "SynthPreInit"},
                {0x832f6b34, "SynthInit"},
                {0x835181f0, "SystemInit"},
                {0x82bc2f80, "MagnuInit"},
                {0x82ae9fc0, "DataInit"},
                {0x83154308, "ObjectDir::PreInit"},
                {0x83151ad8, "ObjectDir::Init"},
                {0x83413cdc, "FileInit"},
                {0x8343b8ac, "LoadMgr::Init"},
                {0x833a3e84, "DxRnd::Init(HWND)"},
                {0x833a40d8, "DxRnd::Init(virt)"},
                {0x8317eeb0, "Rnd::Init(virt)"},
                {0x8356cb88, "DingoServer::Init"},
                {0x82b1de50, "RockCentral::Init"},
                {0x82a37ce8, "HamUserMgrInit"},
                {0x83085950, "FlowInit"},
                {0x83123e60, "CharInit"},
                {0x834f7864, "SaveLoadManager::Init"},
                ...
                {0x8300abe0, "App::DrawRegular"},
                {0x8300b0d8, "MainThread"},
                // Present pipeline
                {0x8339fbd0, "DxRnd::Present"},
                {0x8301bc40, "Game::PostUpdate"},
                {0x837eaabc, "D3DDevice_Swap"},
                // Current decomp-only loop / telemetry hotspots
                {0x83941440, "PIXAddPixOpWithData"},
                {0x8256edc8, "NavListSortMgr::ClearHeaders"},
                {0x8256eee8, "NavListSortMgr::GetFirstChild"},
              };
