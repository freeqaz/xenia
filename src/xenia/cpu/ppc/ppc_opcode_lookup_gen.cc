...
// NOTE (fork): the PPC_DECODER_MISS macro below has been hand-edited relative
// to what ./tools/ppc-table-gen emits (see tools/ppc-table-gen:429). Regenerating
// this file will silently restore the generator's `assert_always()` form, which
// is fatal for the decomp titles under the Checked config. Update the generator
// too if you regenerate.
// clang-format off
#include <cstdint>
#include <cstdlib>
...
// Upstream this was `assert_always()`. The fork removed it outright, which made
// a decoder miss completely silent (docs/fork-cleanup-review.md, section 2).
// kInvalid is handled downstream so continuing is correct -- but it must not be
// mute. Rate-limited to a fixed per-process budget: a mis-scanned function can
// miss on every word.
static void ReportDecoderMiss(uint32_t code) {
  static constexpr uint32_t kDecoderMissLogLimit = 64;
  static std::atomic<uint32_t> miss_count{0};
  uint32_t n = miss_count.fetch_add(1, std::memory_order_relaxed);
  if (n < kDecoderMissLogLimit) {
    XELOGE("PPC decoder miss: {:08X} decodes to no known opcode", code);
  } else if (n == kDecoderMissLogLimit) {
    XELOGE(
        "PPC decoder miss: {:08X} (hit {} times; suppressing further reports)",
        code, kDecoderMissLogLimit + 1);
  }
}

#define PPC_DECODER_MISS ReportDecoderMiss(code); return PPCOpcode::kInvalid
