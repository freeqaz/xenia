...
class SharedMemory {
 public:
  ...
  // When true, MakeRangeValid will not set up mprotect-based page watches
  // via EnablePhysicalMemoryAccessCallbacks. Used in headless mode during
  // one-shot deferred rendering to avoid persistent page watches that
  // interfere with the game thread via SIGSEGV handling.
  void set_suppress_memory_watches(bool suppress) {
    suppress_memory_watches_ = suppress;
  }
 ...
 private:
  ...
  // When true, MakeRangeValid skips EnablePhysicalMemoryAccessCallbacks.
  bool suppress_memory_watches_ = false;
