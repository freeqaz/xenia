...
dword_result_t XamContentGetDeviceData_entry(
  ...
  XELOGD("XamContentGetDeviceData: device_id=0x{:08X}", uint32_t(device_id));
  auto device_info = GetDummyDeviceInfo(device_id);
  if (device_info == nullptr) {
    XELOGD("XamContentGetDeviceData: device not found!");
    return X_ERROR_DEVICE_NOT_CONNECTED;
  }
  device_data.Zero();
...
dword_result_t XamContentCreateDeviceEnumerator_entry(dword_t content_type,
  ...
  XELOGD("XamContentCreateDeviceEnumerator: type={} flags={} max={}",
